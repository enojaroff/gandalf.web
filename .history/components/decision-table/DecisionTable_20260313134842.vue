      ...
      <tbody>
        ...
        >
          ...
          >
            ...
            -->
            <UPopover
              :key="popoverKey(rule._id, condIdx)"
              modal
              :dismissible="true"
              :content="{align: 'start', side: 'right', sideOffset: 0}"
...
// Clés de remontage des UPopover (pour fermeture programmatique via :key)
const popoverKeys = reactive<Record<string, number>>({})

function popoverKey(ruleId: string, condIdx: number): string {
  const k = `${ruleId}:${condIdx}`
  return `${k}-${popoverKeys[k] ?? 0}`
}

function bumpPopoverKey(ruleId: string, condIdx: number) {
  const k = `${ruleId}:${condIdx}`
  popoverKeys[k] = (popoverKeys[k] ?? 0) + 1
}

// Appelé via @pointerdown sur le trigger, avant que le popover rende son contenu
function openEdit(ruleId: string, condIdx: number) {
  if (editingCell.value?.ruleId === ruleId && editingCell.value?.condIdx === condIdx) return
...
// Appelé par @update:open=false (clic extérieur géré nativement par UPopover)
function cancelEdit() {
  editingCell.value = null
  editingConditionCopy.value = null
}

// Bouton Cancel : annule + force la fermeture du popover via remontage
function onCancelClick(ruleId: string, condIdx: number) {
  cancelEdit()
  bumpPopoverKey(ruleId, condIdx)
}

// Bouton Save : sauvegarde + force la fermeture du popover via remontage
function onSaveClick(ruleId: string, condIdx: number) {
  if (editingCell.value && editingConditionCopy.value) {
    const rule = props.variant.rules.find(r => r._id === ruleId)
    if (rule) {
